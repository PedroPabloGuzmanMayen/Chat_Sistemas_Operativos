...
void sendMessage(struct lws *wsi, string reciever, string content) {
    ...
    if (!sender) {
        ...
        serverLogger.log("ERROR, no se pudo encontrar al usuario que envía el mensaje");
        lws_write(wsi, buffer, 2, LWS_WRITE_BINARY);
        return;
    }
    ...
    if (sender->status == DISCONNECTED) {
        buffer[0] = ERROR;
        buffer[1] = USER_DISCONNECTED;
        serverLogger.log("ERROR, el usuario " + sender->username + " estaba desconectado y no pudo enviar el mensaje");
        lws_write(wsi, buffer, 2, LWS_WRITE_BINARY);
        return;
    }
